...
if [ -z "$rlimit_core" -o "0" = "$rlimit_core" ]; then # 0 means disable coredump
  log -p w -t ${TAG} "rlimit_core=${rlimit_core} exit"
  exit 1
fi
#}
...
# classify the proc
fpostfix="app"
# 0xfffffffe is system_server 0xfffffffe 0xfffffffd is zygote
if [ $rlimit_core -eq 4294967294 ] || [ $rlimit_core -eq 4294967293 ]; then
    fpostfix="system"
fi
...
if [ "$fpostfix" != "system" ]; then
    ...
    if [ -n $loglist -a ${#loglist[@]} -gt 0 ]; then
        ...
        if [ $(($ts - $lastTime)) -lt $LIMIT_TIME ]; then
            ...
            log -p w -t ${TAG} "Skip! It had done core dump within ${LIMIT_TIME} seconds ago."
            exit 1
        fi
    fi
fi
...
log -p w -t ${TAG} "Generating core files: ${pid} ..."
